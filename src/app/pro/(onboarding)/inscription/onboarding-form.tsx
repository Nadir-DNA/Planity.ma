...
export default function OnboardingForm({ currentUser }: { currentUser: CurrentUser | null }) {
  const router = useRouter();
  const [step, setStep] = useState(0);
  const [error, setError] = useState("");
  const [isPending, startTransition] = useTransition();

  // Compte propriétaire (uniquement si non connecté)
    ...
    startTransition(async () => {
      try {
        const result = await completeProOnboarding({
          // Infos compte : ignorées côté serveur si une session existe
          ...(currentUser
            ? {}
            : {
          ...
          salonEmail: salonInfo.email || undefined,
        ...
        // Activation par abonnement : redirection vers le checkout Dodo
        if (result.checkoutUrl) {
          window.location.href = result.checkoutUrl;
          return;
        }

        // Checkout indisponible → dashboard avec bannière "paiement requis"
        router.push("/pro?paiement=requis");
      } catch {
  ...
  const isStepValid = () => {
    switch (step) {
      case 0:
        return Boolean(
          accountValid &&
          salonInfo.name && salonInfo.category && salonInfo.address &&
          salonInfo.city && salonInfo.phone
        );
      case 1:
  ...
  return (
    <div>
      ...
      {/* Step 1: Account (si non connecté) + Salon info */}
      {step === 0 && (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-gray-400 flex items-center">
                  ...
                  Déjà un compte ?{" "}
                  <a href="/connexion?callbackUrl=/pro/inscription" className="ml-1 text-rose-600 hover:underline">
                    Connectez-vous
                  </a>
                </p>
              </CardContent>
          ...
          <CardContent>
            ...
            <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm font-medium text-blue-800">
                Dernière étape : l&apos;abonnement Planity.ma
              </p>
              <p className="text-sm text-blue-600 mt-1">
                Cliquez sur &quot;Continuer vers le paiement&quot; pour souscrire
                votre abonnement. Votre salon sera visible et réservable en ligne
      ...
      <div className="flex justify-between mt-6">
        ...
        >
          {step === 3 ? (
            <>
              {isPending ? "Création..." : "Continuer vers le paiement"}
